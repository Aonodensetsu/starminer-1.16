...
public enum GravityDirection
{
  ...
  public static final Vec3 rotateVec3(GravityDirection dir, Vec3 vec3) {
    ...
    vec3.xCoord = x * dir.matrixRotationD[0] + y * dir.matrixRotationD[3] + z * dir.matrixRotationD[6];
    vec3.yCoord = x * dir.matrixRotationD[1] + y * dir.matrixRotationD[4] + z * dir.matrixRotationD[7];
    vec3.zCoord = x * dir.matrixRotationD[2] + y * dir.matrixRotationD[5] + z * dir.matrixRotationD[8];

    return vec3;
  }
  
  public static final Vec3 rotateVec3At(GravityDirection dir, Vec3 vec3, Vec3 centerVec3) {
    return rotateVec3At(dir, centerVec3, centerVec3.xCoord, centerVec3.yCoord, centerVec3.zCoord);
  }
  
  public static final Vec3 rotateVec3At(GravityDirection dir, Vec3 vec3, double centerX, double centerY, double centerZ) {
    double x = vec3.xCoord - centerX;
    double y = vec3.yCoord - centerY;
    double z = vec3.zCoord - centerZ;
    
    vec3.xCoord = x * dir.matrixRotationD[0] + y * dir.matrixRotationD[3] + z * dir.matrixRotationD[6] + centerX;
    vec3.yCoord = x * dir.matrixRotationD[1] + y * dir.matrixRotationD[4] + z * dir.matrixRotationD[7] + centerY;
    vec3.zCoord = x * dir.matrixRotationD[2] + y * dir.matrixRotationD[5] + z * dir.matrixRotationD[8] + centerZ;
  ...
  public static final AxisAlignedBB rotateAABBAt(GravityDirection dir, AxisAlignedBB aabb, double roatCenterX, double roatCenterY, double roatCenterZ) {
    ...
    aabb.minX = Math.min(x1, x2);
    aabb.minY = Math.min(y1, y2);
    aabb.minZ = Math.min(z1, z2);
    
    aabb.maxX = Math.max(x1, x2);
    aabb.maxY = Math.max(y1, y2);
    aabb.maxZ = Math.max(z1, z2);
